...
def _build_main_menu_greeting_text(lang: str) -> str:
    if lang == "ru":
        ru_text = Text(
            Text("🌐 "),
            Bold("VPN\\•PRO"),
            Text(" надёжный помощник в мире безграничного интернета!\n\n"),
            Text("📡 Приватное и быстрое "),
            Bold("VPN-подключение"),
            Text(" без лишних сложностей.\n\n"),
            Text("💬 "),
            TextLink("Служба поддержки", url="https://t.me/rusys"),
            Text("\n\n"),
            Text("Панель управления ⤵️"),
        ).as_markdown()
        return ru_text.replace("\\\\•", "\\•")
    en_text = Text(
        Text("🌐 "),
        Bold("VPN\\•PRO"),
        Text(" is your reliable assistant in the world of limitless internet!\n\n"),
        Text("📡 Private and fast "),
        Bold("VPN connection"),
        Text(" without extra hassle.\n\n"),
        Text("💬 "),
        TextLink("Support", url="https://t.me/rusys"),
        Text("\n\n"),
        Text("Control panel ⤵️"),
    ).as_markdown()
    return en_text.replace("\\\\•", "\\•")


MAIN_MENU_GREETING_FALLBACKS: Dict[str, str] = {
    "ru": _build_main_menu_greeting_text("ru"),
    "en": _build_main_menu_greeting_text("en"),
}
...
async def send_main_menu(target_event: Union[types.Message,
    ...
    if text == "main_menu_greeting":
        text = _get_fallback_text(MAIN_MENU_GREETING_FALLBACKS, current_lang)
